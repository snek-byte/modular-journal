...
export function DraggableIcon({
  ...
  useEffect(() => {
    ...
    const handleDragStart = (e: DragEvent) => {
      ...
      // Store the icon ID in the dataTransfer
      e.dataTransfer.setData("text/plain", id)

      // Calculate offset from the mouse position to the icon's top-left corner
  ...
  // Handle icon click to select it
  ...
  // Update the isPreColoredIcon check to work with our simplified icon set

  // Determine if this is a colored icon that shouldn't have its color changed
  const isPreColoredIcon =
  ...
  return (
    <div
      ref={iconRef}
      className={`icon-item ${isSelected ? "ring-1 ring-blue-500/30" : ""}`}
      style={{
        position: "absolute",
        left: `${position.x}px`,
        top: `${position.y}px`,
    ...
    >
      <div className="select-none" style={{ width: `${size}px`, height: `${size}px` }}>
        {React.createElement(IconComponent, { size, color })}
      </div>
